...
class Plotter():
    ...
    def make_summary_plot(self, out_dir: str, errors: dict, model_name: str, rec_type: str, hbar_names: bool = False):
        ...
        g1 = sns.barplot(data=error_dfs['main'], x='Delta mAP', y='Error Type', ax=ax,
                         palette=self.colors_main.values())

        for i, (name, dmap) in enumerate(zip(error_dfs['main']['Error Type'], error_dfs['main']['Delta mAP'])):
            g1.text(dmap, i, str(round(dmap, 2))+'%', ha="left", va="center", fontdict=font)
        ...
        g2 = sns.barplot(data=error_dfs['special'], x='Error Type', y='Delta mAP', ax=ax,
                         palette=self.colors_special.values())
        for i, (name, dmap) in enumerate(zip(error_dfs['special']['Error Type'], error_dfs['special']['Delta mAP'])):
            g2.text(i, dmap, str(round(dmap, 2)) + '%', ha="center", va="bottom", fontdict=font)
